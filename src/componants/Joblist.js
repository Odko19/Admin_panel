...
function Joblist() {
  ...
  function handlerBtnDlt(id) {
    ...
    fetch(
      `${process.env.REACT_APP_BASE_URL}/workplace/?id=${id}`,
      requestOptions
    )
      ...
      render: (record) =>
        camecv.map((e, i) => {
          if (e.cv_workplace_id === record.workplace_id) {
            return (
              <Badge key={i} count={e.cv_name.length} style={{}}>
                <Button
                  type="primary"
                  onClick={() => showModal(record.workplace_id)}
                >
                  Ирсэн CV
                </Button>
              </Badge>
      ...
      {select ? (
        ...
        <div>
          ...
          <Button
            ...
            onClick={() =>
              (window.location.href = `${process.env.REACT_APP_PUSH_URL}/humanresources`)
            }
            type="primary"
          >
            Вэб дээр харах
