...
// Delete
  function handleDltRes(record) {
    ...
    fetch(`${process.env.REACT_APP_BASE_URL}/coverimg/?id=${record.id}`, requestOptions)
      .then((response) => response.json())
      .then((result) => {
        if (result.success === true) {
          openNotification("success");
          fetch(`${process.env.REACT_APP_BASE_URL}/coverimg/?type=${record.type}`)
            .then((response) => response.json())
            .then((result) => {
              if(record.type==="resident"){setResident(
...
//
  ...
  return (
    <div>
      <Button  onClick={()=>handleUpload()} type="primary" style={{width : "200px",}}>Зураг оруулах</Button>  <Tag style={{marginLeft: "20px"}} color="orange">Зургийн хэмжээ 1900px / 680px байна</Tag>
  ...
  <Table
        ...
        <Modal
          ...
          <Radio.Group value={modaldata?.type} onChange={placementChange} style={{ width: "50%"}}>
  <Radio.Button value="resident" style={{ width: "50%" }}>  Хувь хүн </Radio.Button>
  <Radio.Button value="business" style={{ width: "50%" }}>  Байгууллага </Radio.Button>
          </Radio.Group>
</div>
      ...
      <br />
    <button type="submit" className="coverBtnSave"> Save</button>
    </section></form>
             </Modal>
    </div>
  )
